class HogsController < ApplicationController
  ...
  def index
    ...
    render json: @hogs.to_json(:include => {:piglets => {:only => [:name, :cuteness]}})
  end

  # GET /hogs/1
  def show
    render json: @hog.to_json(:include => {:piglets => {:only => [:name, :cuteness]}})
  end
  ...
  private
    ...
    def hog_params
      params.require(:hog).permit(:name, :specialty, :greased, :weight, :medal, :image)
    end
end
